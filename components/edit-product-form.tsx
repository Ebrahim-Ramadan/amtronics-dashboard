...
export function EditProductForm({ product, onSuccess, onClose }: EditProductFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [id]: id === "price" || id === "quantity_on_hand" || id === "barcode" || id === "id" || id === "sold_quantity" ? Number(value) : value,
    }))
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
      ...
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <Input id="barcode" type="number" value={formData.barcode} onChange={handleChange} className="col-span-3" required />
      </div>
      <div className="grid grid-cols-4 items-center gap-4">
      ...
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <Input id="sold_quantity" type="number" value={formData.sold_quantity} onChange={handleChange} className="col-span-3" required />
      </div>
