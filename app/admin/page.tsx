...
export default function AdminManagementPage() {
  ...
  // For engineer's projects and sub-user's data
  ...
  // Fetch data for engineer or sub
  useEffect(() => {
    if (session?.role === "engineer") {
      fetchEngineerProjects(session.email);
    } else if (session?.role === "sub") {
      fetchSubUserData(session.allowedProjects || [], session.allowedPromos || []);
    }
  }, [session]);
  ...
  async function fetchSubUserData(projectIds: string[], promoIds: string[]) {
    setUserLoading(true);
    console.log('promoIds', promoIds);
    
    try {
      const [projectsRes, promosRes] = await Promise.all([
        fetch(`/api/projects?ids=${projectIds.join(',')}`),
        fetch(`/api/promocodes?ids=${promoIds.join(',')}`)
      ]);
      if (projectsRes.ok) {
            ...
            <div>
              ...
              <div className="overflow-x-auto w-full">
                <table className="min-w-full text-xs md:text-sm whitespace-nowrap">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="px-2 py-2 text-left">Promo Code</th>
                  ...
                  <tbody>
                    {promos.map((p) => (
                      <tr key={p._id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-2 py-2">{p.percentage ? `${p.percentage}% off` : "No discount"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
    ...
    <div className="min-h-screen bg-gray-50 p-2 md:p-6">
      <div className="w-full mx-auto space-y-6">
        ...
        <div>
          ...
          <form className="flex flex-col gap-2 mb-4" onSubmit={addUser}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
                        {allPromos.map(p => (
                          <button
                            key={p._id}
                            type="button"
                            className={`relative border rounded-lg px-4 py-3 text-left flex flex-col justify-between items-start transition-all duration-200 ease-in-out shadow-sm
