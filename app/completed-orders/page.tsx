...
export default function CompletedOrdersDashboard({ searchParams }: PageProps) {
  const suspenseKey = getSuspenseKey(searchParams, "completed")
      ...
      <div className="w-full mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-2 md:gap-4">
            {/* Dropdown Menu */}
            <Topleftmenu/>
            <h1 className="text-xl md:text-3xl font-bold text-gray-900">Completed Orders Dashboard</h1>
          </div>
        ...
        <Suspense key={suspenseKey} fallback={isInitialLoad ? <LoadingSkeleton /> : <SortingLoadingSkeleton />}>
          <OrdersDashboardContent searchParams={searchParams} initialStatus="completed" />
        </Suspense>
      </div>
    </div>
  )
